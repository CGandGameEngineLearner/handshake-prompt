...
传统做法通常意味着配置 API Key、开通系统权限、建立长期信任关系。  
但很多协作其实只是一次性的：**这次任务、这个范围、这段时间内允许你做这些事。**
...
Agent A 生成握手提示词
      │
      │ 其中包含：sessionId、token、可调用能力、有效期、边界说明
      ↓
Agent B 读取握手提示词
      │
      │ 在 token 有效期内调用 Agent A 暴露的受限能力
      ↓
任务完成 / Token 过期 / 会话关闭
      │
      ↓
